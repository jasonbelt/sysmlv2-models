...
object TempControl_tcp_tct {
  ...
  def initialise(api: TempControl_Initialization_Api): Unit = {
    ...
    fanError = F
    currentSetPoint = Defs.defaultSetPoint
    currentFanState = FanCmd.Off
  ...
  def perform_fan_control(api: TempControl_Operational_Api) : Unit = {
    ...
    if (fanError) {
      ...
      println(st"""$oldFanState != $currentFanState = ${currentFanState != oldFanState}""".render)
      println(st"""${oldFanState.name} != ${currentFanState.name} = ${currentFanState.name != oldFanState.name}""".render)
