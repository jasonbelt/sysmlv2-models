...
import tc.TempControl_Inputs_Container
import tc.TempControl_Outputs_Container
...
object JSON {
  ...
  object Printer {
    ...
    @pure def printTempControl_Inputs_Container(o: TempControl_Inputs_Container): ST = {
      return printObject(ISZ(
        ("type", st""""TempControl_Inputs_Container""""),
        ("currentTemp", printTempControlMixedSeL4CAmKESTemperature(o.currentTemp)),
        ("tempChanged", printOption(F, o.tempChanged, print_artEmpty _)),
        ("fanError", printB(o.fanError))
      ))
    }

    @pure def printTempControl_Outputs_Container(o: TempControl_Outputs_Container): ST = {
      return printObject(ISZ(
        ("type", st""""TempControl_Outputs_Container""""),
        ("fanCmd", printOption(F, o.fanCmd, printTempControlMixedSeL4CAmKESFanCmdType _)),
        ("fanAck", printOption(F, o.fanAck, printTempControlMixedSeL4CAmKESFanAckType _)),
        ("fanIsOn", printB(o.fanIsOn)),
        ("currentFanState", printTempControlMixedSeL4CAmKESFanCmdType(o.currentFanState)),
        ("currentSetPoint", printTempControlMixedSeL4CAmKESSetPoint(o.currentSetPoint))
      ))
    }
  ...
  @record class Parser(val input: String) {
    ...
    def parseTempControl_Inputs_Container(): TempControl_Inputs_Container = {
      val r = parseTempControl_Inputs_ContainerT(F)
      return r
    }

    def parseTempControl_Inputs_ContainerT(typeParsed: B): TempControl_Inputs_Container = {
      if (!typeParsed) {
        parser.parseObjectType("TempControl_Inputs_Container")
      }
      parser.parseObjectKey("currentTemp")
      val currentTemp = parseTempControlMixedSeL4CAmKESTemperature()
      parser.parseObjectNext()
      parser.parseObjectKey("tempChanged")
      val tempChanged = parser.parseOption(parse_artEmpty _)
      parser.parseObjectNext()
      parser.parseObjectKey("fanError")
      val fanError = parser.parseB()
      parser.parseObjectNext()
      return TempControl_Inputs_Container(currentTemp, tempChanged, fanError)
    }

    def parseTempControl_Outputs_Container(): TempControl_Outputs_Container = {
      val r = parseTempControl_Outputs_ContainerT(F)
      return r
    }

    def parseTempControl_Outputs_ContainerT(typeParsed: B): TempControl_Outputs_Container = {
      if (!typeParsed) {
        parser.parseObjectType("TempControl_Outputs_Container")
      }
      parser.parseObjectKey("fanCmd")
      val fanCmd = parser.parseOption(parseTempControlMixedSeL4CAmKESFanCmdType _)
      parser.parseObjectNext()
      parser.parseObjectKey("fanAck")
      val fanAck = parser.parseOption(parseTempControlMixedSeL4CAmKESFanAckType _)
      parser.parseObjectNext()
      parser.parseObjectKey("fanIsOn")
      val fanIsOn = parser.parseB()
      parser.parseObjectNext()
      parser.parseObjectKey("currentFanState")
      val currentFanState = parseTempControlMixedSeL4CAmKESFanCmdType()
      parser.parseObjectNext()
      ...
      val currentSetPoint = parseTempControlMixedSeL4CAmKESSetPoint()
      parser.parseObjectNext()
      return TempControl_Outputs_Container(fanCmd, fanAck, fanIsOn, currentFanState, currentSetPoint)
    }
  ...
  def fromTempControl_Inputs_Container(o: TempControl_Inputs_Container, isCompact: B): String = {
    val st = Printer.printTempControl_Inputs_Container(o)
    if (isCompact) {
      return st.renderCompact
    } else {
      return st.render
  ...
  def toTempControl_Inputs_Container(s: String): Either[TempControl_Inputs_Container, Json.ErrorMsg] = {
    def fTempControl_Inputs_Container(parser: Parser): TempControl_Inputs_Container = {
      val r = parser.parseTempControl_Inputs_Container()
      return r
    }
    val r = to(s, fTempControl_Inputs_Container _)
    return r
  }

  def fromTempControl_Outputs_Container(o: TempControl_Outputs_Container, isCompact: B): String = {
    val st = Printer.printTempControl_Outputs_Container(o)
    if (isCompact) {
      return st.renderCompact
    } else {
      return st.render
  ...
  def toTempControl_Outputs_Container(s: String): Either[TempControl_Outputs_Container, Json.ErrorMsg] = {
    def fTempControl_Outputs_Container(parser: Parser): TempControl_Outputs_Container = {
      val r = parser.parseTempControl_Outputs_Container()
      return r
    }
    val r = to(s, fTempControl_Outputs_Container _)
    return r
  }
