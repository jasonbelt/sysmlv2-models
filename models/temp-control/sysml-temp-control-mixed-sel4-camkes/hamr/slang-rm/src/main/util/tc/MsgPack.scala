...
object MsgPack {
  ...
  object Constants {
    ...
    val TempControl_Inputs_Container: Z = 5

    val TempControl_Outputs_Container: Z = 6
  ...
  @msig trait Writer {
    ...
    def writeTempControl_Inputs_Container(o: TempControl_Inputs_Container): Unit = {
      writer.writeZ(Constants.TempControl_Inputs_Container)
      writeTempControlMixedSeL4CAmKESTemperature(o.currentTemp)
      writer.writeOption(o.tempChanged, write_artEmpty _)
      writer.writeB(o.fanError)
    }

    def writeTempControl_Outputs_Container(o: TempControl_Outputs_Container): Unit = {
      writer.writeZ(Constants.TempControl_Outputs_Container)
      writer.writeOption(o.fanCmd, writeTempControlMixedSeL4CAmKESFanCmdType _)
      writer.writeOption(o.fanAck, writeTempControlMixedSeL4CAmKESFanAckType _)
      writer.writeB(o.fanIsOn)
      writeTempControlMixedSeL4CAmKESFanCmdType(o.currentFanState)
      writeTempControlMixedSeL4CAmKESSetPoint(o.currentSetPoint)
    }
  ...
  @msig trait Reader {
    ...
    def readTempControl_Inputs_Container(): TempControl_Inputs_Container = {
      val r = readTempControl_Inputs_ContainerT(F)
      return r
    }

    def readTempControl_Inputs_ContainerT(typeParsed: B): TempControl_Inputs_Container = {
      if (!typeParsed) {
        reader.expectZ(Constants.TempControl_Inputs_Container)
      }
      val currentTemp = readTempControlMixedSeL4CAmKESTemperature()
      val tempChanged = reader.readOption(read_artEmpty _)
      val fanError = reader.readB()
      return TempControl_Inputs_Container(currentTemp, tempChanged, fanError)
    }

    def readTempControl_Outputs_Container(): TempControl_Outputs_Container = {
      val r = readTempControl_Outputs_ContainerT(F)
      return r
    }

    def readTempControl_Outputs_ContainerT(typeParsed: B): TempControl_Outputs_Container = {
      if (!typeParsed) {
        reader.expectZ(Constants.TempControl_Outputs_Container)
      }
      val fanCmd = reader.readOption(readTempControlMixedSeL4CAmKESFanCmdType _)
      val fanAck = reader.readOption(readTempControlMixedSeL4CAmKESFanAckType _)
      val fanIsOn = reader.readB()
      val currentFanState = readTempControlMixedSeL4CAmKESFanCmdType()
      val currentSetPoint = readTempControlMixedSeL4CAmKESSetPoint()
  ...
  def fromTempControl_Inputs_Container(o: TempControl_Inputs_Container, pooling: B): ISZ[U8] = {
    val w = Writer.Default(MessagePack.writer(pooling))
    w.writeTempControl_Inputs_Container(o)
    return w.result
  }

  def toTempControl_Inputs_Container(data: ISZ[U8]): Either[TempControl_Inputs_Container, MessagePack.ErrorMsg] = {
    def fTempControl_Inputs_Container(reader: Reader): TempControl_Inputs_Container = {
      val r = reader.readTempControl_Inputs_Container()
      return r
    }
    val r = to(data, fTempControl_Inputs_Container _)
    return r
  }

  def fromTempControl_Outputs_Container(o: TempControl_Outputs_Container, pooling: B): ISZ[U8] = {
    val w = Writer.Default(MessagePack.writer(pooling))
    w.writeTempControl_Outputs_Container(o)
    return w.result
  }

  def toTempControl_Outputs_Container(data: ISZ[U8]): Either[TempControl_Outputs_Container, MessagePack.ErrorMsg] = {
    def fTempControl_Outputs_Container(reader: Reader): TempControl_Outputs_Container = {
      val r = reader.readTempControl_Outputs_Container()
      return r
    }
    val r = to(data, fTempControl_Outputs_Container _)
    return r
  }
