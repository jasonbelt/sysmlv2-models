...
@msig trait RandomLibI {
  ...
  // ============= TempControl_Inputs_Container ===================

  def get_Config_TempControl_Inputs_Container: Config_TempControl_Inputs_Container
  def set_Config_TempControl_Inputs_Container(config: Config_TempControl_Inputs_Container): RandomLib

  def nextTempControl_Inputs_Container(): TempControl_Inputs_Container = {
    var currentTemp: TempControlMixedSeL4CAmKES.Temperature = nextTempControlMixedSeL4CAmKESTemperature()
    var tempChanged: Option[art.Empty] = nextOption_artEmpty()
    var fanError: B = nextB()

    var v: TempControl_Inputs_Container = TempControl_Inputs_Container(currentTemp, tempChanged, fanError)

    if(get_Config_TempControl_Inputs_Container.attempts >= 0) {
     for(i <- 0 to get_Config_TempControl_Inputs_Container.attempts) {
        if(get_Config_TempControl_Inputs_Container.filter(v)) {
          return v
        }
        if (get_Config_TempControl_Inputs_Container.verbose) {
          println(s"Retrying for failing value: $v")
        }
        currentTemp = nextTempControlMixedSeL4CAmKESTemperature()
        tempChanged = nextOption_artEmpty()
        fanError = nextB()
        v = TempControl_Inputs_Container(currentTemp, tempChanged, fanError)
     }
    } else {
     while(T) {
       if(get_Config_TempControl_Inputs_Container.filter(v)) {
  ...
  // ============= TempControl_Outputs_Container ===================

  def get_Config_TempControl_Outputs_Container: Config_TempControl_Outputs_Container
  def set_Config_TempControl_Outputs_Container(config: Config_TempControl_Outputs_Container): RandomLib

  def nextTempControl_Outputs_Container(): TempControl_Outputs_Container = {
    var fanCmd: Option[TempControlMixedSeL4CAmKES.FanCmd.Type] = nextOptionTempControlMixedSeL4CAmKESFanCmdType()
    var fanAck: Option[TempControlMixedSeL4CAmKES.FanAck.Type] = nextOptionTempControlMixedSeL4CAmKESFanAckType()
    var fanIsOn: B = nextB()
    ...
    var v: TempControl_Outputs_Container = TempControl_Outputs_Container(fanCmd, fanAck, fanIsOn, currentFanState, currentSetPoint)

    if(get_Config_TempControl_Outputs_Container.attempts >= 0) {
     for(i <- 0 to get_Config_TempControl_Outputs_Container.attempts) {
        if(get_Config_TempControl_Outputs_Container.filter(v)) {
          return v
        }
        if (get_Config_TempControl_Outputs_Container.verbose) {
          println(s"Retrying for failing value: $v")
        }
        ...
        v = TempControl_Outputs_Container(fanCmd, fanAck, fanIsOn, currentFanState, currentSetPoint)
     }
    } else {
     while(T) {
       if(get_Config_TempControl_Outputs_Container.filter(v)) {
...
@record class RandomLib(val gen: org.sireum.Random.Gen) extends RandomLibI {
  ...
  // ============= TempControl_Inputs_Container ===================
  def alwaysTrue_TempControl_Inputs_Container(v: TempControl_Inputs_Container): B = {return T}

  var config_TempControl_Inputs_Container: Config_TempControl_Inputs_Container = Config_TempControl_Inputs_Container(100, _verbose, alwaysTrue_TempControl_Inputs_Container _)

  def get_Config_TempControl_Inputs_Container: Config_TempControl_Inputs_Container = {return config_TempControl_Inputs_Container}

  def set_Config_TempControl_Inputs_Container(config: Config_TempControl_Inputs_Container): RandomLib ={
    config_TempControl_Inputs_Container = config
    return this
  }
  ...
  // ============= TempControl_Outputs_Container ===================
  def alwaysTrue_TempControl_Outputs_Container(v: TempControl_Outputs_Container): B = {return T}

  var config_TempControl_Outputs_Container: Config_TempControl_Outputs_Container = Config_TempControl_Outputs_Container(100, _verbose, alwaysTrue_TempControl_Outputs_Container _)

  def get_Config_TempControl_Outputs_Container: Config_TempControl_Outputs_Container = {return config_TempControl_Outputs_Container}

  def set_Config_TempControl_Outputs_Container(config: Config_TempControl_Outputs_Container): RandomLib ={
    config_TempControl_Outputs_Container = config
    return this
  }
