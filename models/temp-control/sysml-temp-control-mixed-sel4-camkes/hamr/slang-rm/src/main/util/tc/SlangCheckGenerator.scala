...
@record class Gen_TempControl_Inputs_Container(param: RandomLibI) extends MJen[TempControl_Inputs_Container] {
  override def generate(f: TempControl_Inputs_Container => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextTempControl_Inputs_Container())
...
@record class Gen_TempControl_Outputs_Container(param: RandomLibI) extends MJen[TempControl_Outputs_Container] {
  override def generate(f: TempControl_Outputs_Container => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextTempControl_Outputs_Container())
